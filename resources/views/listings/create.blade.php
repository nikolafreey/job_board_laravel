            ...
            <form action="{{ route('listings.store') }}"
                ...
                @guest
                     <div class="flex mb-4">
                        <div class="flex-1 mx-2">
                            ...
                            <x-input class="block mt-1 w-full" id="email" name="email" type="email" :value="old('email')" required autofocus />
                        </div>
                        <div class="flex-1 mx-2">
                            <x-label for="name" value="Full Name" />
                            <x-input class="block mt-1 w-full" id="name" name="name" type="text" :value="old('name')" required />
                        </div>
                    </div>
                    <div class="flex mb-4">
                        <div class="flex-1 mx-2">
                            <x-label for="password" value="Password" />
                            <x-input class="block mt-1 w-full" id="password" name="password" type="password" required />
                        </div>
                        <div class="flex-1 mx-2">
                            <x-label for="confirm_password" value="Confirm Password" />
                            <x-input class="block mt-1 w-full" id="confirm_password" name="confirm_password" type="password" required />
                        </div>
                    </div>  
                @endguest
                <div class="mb-4 mx-2">
                ...
                <div class="mb-2 mx-2">
                    ...
                    <input 
                        ...
                        value=""
                    />
                    <button type="submit" id="form_submit" class="block w-full items-center bg-indigo-500 text-white border-0 py-2 focus:outline-none hover:bg-indigo-600 rounded text-base mt-4 md:mt-0">
                        Pay + Continue
                    </button>
                </div>
            </form>
    ...
    <script>
        ...
        document.getElementById('form-submit').addEventListener('click', async (e) => {
            ...
            if(error){
                ...
                dockument.getElementById('payment_form').submit();
            }
        });
    </script>
</x-app-layout>